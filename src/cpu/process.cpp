        ...
        for(int i=0; i<jdata["file"].size(); i++){
            try{
                ...
                if (!success) {
                    ...
                    homographySurf.setReference(old_reference);
                    success = homographySurf.warpImage(GPU);
                    use_old = true;
                ...
                if (use_old) {
                    _imageFileReference = oldFileReference;
                    use_old = false;
                }
                else _imageFileReference = imageFileReference;
                if (!success || (homographySurf.getMove() && !IsMorning(taken_time))){
					// log json
					json obj;
					obj["name"] = name;
                    ...
                    //log
                    log = "unsuccess    "+ name + "    "+ _imageFileReference + "    "+ to_string(n) +"      " +to_string(process_time)+"\n";
                    logfile << log;
                ...
                if (homographySurf.getMove() && IsMorning(taken_time)){      // co chuyen goc
                    ...
                    //log
                    log = "success   "+ name +"     "+ "move image with     "+ _imageFileReference + to_string(process_time)+"\n";
                    ...
                    //log
                    log = "success    "+ name + "    "+ _imageFileReference + "    "+ to_string(n) +"      " + to_string(process_time)+"\n";
                }
